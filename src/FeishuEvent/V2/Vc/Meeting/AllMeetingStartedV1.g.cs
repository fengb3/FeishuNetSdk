...
namespace FeishuNetSdk.FeishuEvent.V2.Vc.Meeting
{
    /// <summary>
    /// 企业会议开始<br/>
    /// 发生在会议开始时，包含企业内所有会议开始事件。<br/>
    /// </summary>
    /// <事件类型> vc.meeting.all_meeting_started_v1 </事件类型>
    /// <事件版本> V2.0 </事件版本>
    /// <自建应用> 支持 </自建应用>
    /// <商店应用> 支持 </商店应用>
    /// <文档链接> <a href="https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting/events/all_meeting_started">在open.feishu.cn中查看</a> </文档链接>
    /// <权限要求>
    /// <ul>
    /// <li> 获取所有视频会议信息 </li>
    /// </ul>
    ...
    /// </ul>
    /// </字段权限要求>
    public class AllMeetingStartedV1 : EventContext<AllMeetingStartedV1.EventData>
    {
        // EventData|meeting:meeting_event_meeting|id:string||topic:string||meeting_no:string||meeting_source:int||start_time:string||end_time:string||host_user:meeting_event_user|id:user_id|union_id:string||user_id:string||open_id:string||user_role:int||user_type:int||owner:meeting_event_user|id:user_id|union_id:string||user_id:string||open_id:string||user_role:int||user_type:int||calendar_event_id:string||operator:meeting_event_user|id:user_id|union_id:string||user_id:string||open_id:string||user_role:int||user_type:int|
        ...
        // meeting_event_meeting|id:string||topic:string||meeting_no:string||meeting_source:int||start_time:string||end_time:string||host_user:meeting_event_user|id:user_id|union_id:string||user_id:string||open_id:string||user_role:int||user_type:int||owner:meeting_event_user|id:user_id|union_id:string||user_id:string||open_id:string||user_role:int||user_type:int||calendar_event_id:string|
        public class MeetingEventMeeting
        {
            ...
            /// 会议创建源
            /// 可选值有：
            /// 1：日程会议2：即时会议3：面试会议4：开放平台会议100：其他会议类型
            /// </summary>
            [global::System.Text.Json.Serialization.JsonPropertyName("meeting_source")]
            public int MeetingSource { get; init; }
            ...
            /// 日程实体的唯一标志
            /// </summary>
            [global::System.Text.Json.Serialization.JsonPropertyName("calendar_event_id")]
